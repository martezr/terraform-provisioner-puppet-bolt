...
func Task(connInfo map[string]string, timeout time.Duration, sudo bool, task string, args map[string]string) (*Result, error) {
	...
	cmdargs = append(cmdargs, "--format", "json", "--connect-timeout", "600", task)
...
// Plan Puppet Bolt execution
func Plan(connInfo map[string]string, timeout time.Duration, sudo bool, plan string, parameters map[string]interface{}, modulepath string, args map[string]string) (*Result, error) {
	cmdargs := []string{
		"bolt", "plan", "run", "--nodes", connInfo["type"] + "://" + connInfo["host"], "-u", connInfo["user"],
	}
	...
	cmdargs = append(cmdargs, "--params", "'"+jsonStr+"'")
	if modulepath != "" {
		cmdargs = append(cmdargs, "--modulepath", modulepath)
	}
	cmdargs = append(cmdargs, "--format", "json", "--connect-timeout", "600", plan)
